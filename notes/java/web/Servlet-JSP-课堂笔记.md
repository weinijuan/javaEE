  ...
  - 代码示范：
        ...
        public class HelloServlet implements Servlet
      {
            public void destroy() 
          {
            ...
            {
                System.out.println("Hello World crm");
            }
        }
      ```
...
- 第八步：将以上编译之后的HelloServlet.class文件拷贝到WEB-INF\classes目录下。注意， 带包名的java程序需要把**包名产生的目录以及class文件**一起放到classes目录中，默认包则不需要
...
- 第九步：在web.xml文件中编写配置信息，让“请求路径”和“Servlet类名”关联在一起。
  ...
  - 超链接的以/开头表示绝对路径，超链接的根目录是webapps，因此超链接也可以访问其他app的应用，比如我们是在oa项目中，使用下面的超链接就可以访问crm项目的对应/fdsa/hello路径的java程序了
  
      ```html
              <a href="/crm/fdsa/hello">hello</a>
      ```
...
- 使用IDEA集成开发工具开发Servlet
  ...
  - 第三步：让Module变成JavaEE的模块。（让Module变成webapp的模块。符合webapp规范。符合Servlet规范的Module）
    ...
    - 在弹出的窗口中，选择Web Application（选择的是webapp的支持， 中文名叫web应用程序）
  ...
  - 第七步：在**WEB-INF目录下**新建了一个子目录：lib（这个目录名可不能随意，必须是全部小写的lib），并且将连接数据库的驱动jar包放到lib目录下。
  ...
  - 第十步：让IDEA工具去关联Tomcat服务器。关联的过程当中将webapp部署到Tomcat服务器当中。
    ...
    - 修改 Application context (应用程序上下文)为：/xmm
  ...
  ​        注意，这个时候直接输入localhost:8080会404,必须要带/xmm才可以，同时不能使用IDEA内置的html显示页面，因为那个访问的不是8080端口的Tomcat， 而是其他软件
...
- ServletContext接口中有哪些常用的方法？
  ...
  - ```java
    ...
    // 通过IDEA记录的日志信息会放到IDEA的备份Tomcat中，可以通过查看IDEA运行时的Tomcat_base找到该目录
    ```
  ```
    ...
    - request对象实际上又称为“请求域”对象。
      ...
      - HttpServletRequest接口的其他常用方法：
        ...
        - ```java
          ...
          > URL包含URI， **URI等于contextPath+ServletPath**
...
- 实现步骤
  ...
  > 特别需要注意，当mysql服务未开启的时候需要， 连接数据库就会出现500错误
...
- 我们的第一个注解：
  ...
  - 在Servlet类上使用：@WebServlet，WebServlet注解中有哪些属性呢？
    ...
    - value属性：当注解的属性名只有value的时候，使用注解的时候，value属性名是可以省略的。value属性的作用就是urlpatterns， 但是value和urlPatterns不能够同时设置。
...
```java
@WebServlet(name = "hello",  value = {"/hello1", "/hello2"},
initParams = {@WebInitParam(name = "name", value = "huangwei"), @WebInitParam(name = "age", value =
        "20")})
public class Hello extends HttpServlet{}

// 很多时候都是这样的，value可以省略不写
@WebServlet("/hello")
public class Hello extends HttpServlet{}
```
...
- 默认情况下session对象在浏览器关闭的时候，会话就会断开，经过一段时间以后，session对象才会被服务器销毁，这段时间可能存在不安全问题。
...
- session的实现原理中，每一个session对象都会关联一个sessionid，例如：
  ...
  - 对于session关联的cookie来说，这个cookie是被保存在浏览器的“运行内存”当中。因此在浏览器关闭以后，这个cookie就会被销毁，与之关联的session对象将只能超时才能销毁
...
```java
cookie.setMaxAge(60*60); //正数,cookie将存放在磁盘中,一小时后销毁
cookie.setMaxAge(0); // 销毁当前cookie
cookie.setMaxAge(-1); // 和默认情况一致，保存在运行内存中，浏览器关闭销毁
```
...
- 如何销毁cookie对象
  ...
  ```java
  cookie.setMaxAge(0);
  // 将无效的cookie对象传递给浏览器，让浏览器删除相应的cookie
  response.addCookie(cookie);
  ```
...
- 关于cookie的path，cookie关联的路径：
  ...
  - 手动设置cookie的path
    ...
    ```java
    cookie.setPath(request.getContextPath()+"/test"); // 这样将设置项目名/test下的都会发送含有cookie的请求,而形如项目名/test1这样的就不会得到该cookie了
    ```
...
> 解决idea工具中没有EL表达式代码提示: 按照servlet时候学的导入相应的servlet和jsp的依赖就可以了
...
- EL表达式的使用：
  ...
  - 空值处理: EL表达式对null进行了预处理。如果是null，则向浏览器输出一个空字符串。

  ```java
  ${user2} 域中没有user2的属性,因此显示为""
  <%=request.getAttribute("user2")%>则显示为"null"
  ```
  ...
  - 通过EL表达式获取应用的根：
    ...
    因为EL表达式只内置了jsp九大内置对象的一个，也就是pageContext， 因此不能直接使用request对象，但是因为pageContext对象有getRequest方法，所以在EL表达式中可以通过pageContext.request方法获得request对象，同理可以获得其他jsp的内置对象
  ...
  - EL表达式中其他的隐式对象：
    ...
    <br>EL表达式的内置对象：
    <br>1. pageContext对象
    <br>获取请求对象${pageContext.request}
    <br>获取session对象${pageContext.session}
    <br>2. param 指 请求中的参数
    <br>获取请求中name的值:
    <br>EL表达式方式:
    <br>${param.name}
    ...
    <br>3. paramValues 对于请求中可能对应多个value的key
    <br>EL表达式方式: ${paramValues.name}
    <br>jsp方式: <%=request.getParameterValues("name")%>
    <br>4. initParam 指ServletContext既application对象设置的参数
    <br>获取爱好EL表达式方式:  ${initParam.hobby}
    <br>获取食物jsp方式：<%=application.getInitParameter("food")%>
    ```

    > 需要在请求中加参数以及在web.xml中加context-param参数
  ...
  - EL表达式的运算符
    ...
    > eq相当于==都调用了equals方法，而!相当于not关键字


```jsp
<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<%--在EL表达式中加号永远只进行算术加法或是可以转为算术加法的运算--%>
<br>${10+20}
<br>${10+"20"}
<br><%--下面这行有问题--%>
<br>\${10+"ab"}
<br>${true && false}
<br>${true and false}
<br><%--empty 判断是否为空，为空则true--%>
<br>${empty null}
<br><%--因为pageContext没有getName方法，所以下面报错--%>
<br>\${empty pageContext.name}
<br><%--如果没有在请求中设置相应参数会导致request.getParameter("name")的返回结果为空--%>
<br>${empty param.name ? "用户名不可以为空" : "欢迎您的光临"}
...
- 一个过滤器怎么写呢
  ...
  public class Filter1 implements Filter
  {
      ...
      {
          // 过滤器执行之前进行控制
          System.out.println("filter execute begin");
          // 传给下一个过滤器，没有则传给servlet
          chain.doFilter(request, response);
          // 过滤器执行之后进行控制
          System.out.println("filter execute end");
      }
  }
  ```
...
- Filter过滤器这里有一个设计模式：
  ...
  - 责任链设计模式最大的核心思想：
    ...
    - 过滤器通常在xml中进行配置，而非使用注解，否则任然需要改动代码
...
- 实现oa项目中当前登录在线的人数。
  ...
  - 什么代表着用户登录了？
    - session.setAttribute("user", new User()); User类型的对象只要往session中存储过，表示有新用户登录。
  - 什么代表着用户退出了？
...
### 联合使用cookie和session机制来实现登录机制以及十天免登录机制

##### 设置欢迎页面、配置session死亡时间
...
功能：“检查cookie是否存在,存在则根据cookie制作session，不存在则去登录注册页面，并产生session和cookie”

与session相比：每次都要根据cookie进行数据库的查询，速度慢，而session则无需查询数据库，速度更快。

可以把session比作一级缓存，cookie比作二级缓存，当一级缓存没有的时候才会去二级缓存中查找并因此生成一级缓存。session是在服务器端的，可以理解为内存，当服务器断开连接（浏览器关闭时）理解为断电，内存中的数据将会丢失。而cookie是在客户端的，相当于磁盘，数据不易失。

```java
package com.weinijuan.dept.servlet;
...
public class WelcomeServlet extends HttpServlet
{
            ...
            IOException
    {
        ...
        if (name != null && password != null)
        {
            ...
            if (ok)
            {
                // cookie正确符合数据库，则将用户信息保存到session域中，作为一个记号
                // 表明登录成功
                HttpSession session = request.getSession();
            ...
            {
                // cookie不正确则需要重新输入用户名和密码
                response.sendRedirect(request.getContextPath() + "/login.jsp");
            }
        } 
        // 如果没有cookie也需要输入用户名和密码
        else
        {
            response.sendRedirect(request.getContextPath() + "/login.jsp");
        }
...
<form action="<%=request.getContextPath()%>/login" method="post">
    ...
    十天内免登录 <input type="checkbox" name="freeLogin" value="true"><br>
...
- 检查表单中的数据，如果不符合则告知其密码和用户名错误

- 如果符合，则新建session,加入user属性表明登录成功
    - 勾选了十天免登录，设置一个cookie给浏览器
    - 未勾选，不设置cookie
...
private void doLogin(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException
    {
        ...
        if (ok)
        {
            HttpSession session = request.getSession();
            session.setAttribute("user", name);
            if ("true".equals(request.getParameter("freeLogin")))
            {
...
- welcome负责检查cookie数据，
- 没有cookie的会被welcome转到登录页面
- 登录页面以后可以自行设置与否cookie数据
- 要想访问主页面，必须先有session对象的user属性，如果没有，一律跳转到welcome中

> 这种结构目前我发现的唯一问题是: 在session对象存在，就是已登录的时候，即使数据库更改了密码，只要session对象还在(浏览器没有关闭),就依旧能访问呢主页面
...
@WebServlet({"/list", "/detail", "/delete", "/edit", "/update", "/add"})
public class DeptServlet extends HttpServlet
{
            ...
            throws ServletException, IOException
    {
        ...
        String contextPath = request.getContextPath();
        // 如果没有session对象，说明登录未成功，跳转到欢迎页面（不能跳转到登录页面，因为要先通过欢迎页面检查cookie对象）
        if (session == null || session.getAttribute("user") == null)
        {
            response.sendRedirect(contextPath + "/welcome");
        }
        // 如果不加else的话,重定向以后还能访问下列的代码，但是因为重定向以后无法转发，所以下面这段代码的执行过程中会出现500错误,所以加个else
//        System.out.println("重定向以后还能访问这段代码");
        else
        {
...
1. 销毁session对象
2. 销毁cookie对象：增加一个名字与路径都相同的并且age为0的cookie对象
...
if (cookies != null)
{
    for (int i = 0; i < cookies.length; i++)
    {
        if ("username".equals(cookies[i].getName()) || "password".equals(cookies[i].getName()))
        {
            ...
            // 这里填写最初cookies设置的path,即cookie产生时的path，默认是上一级目录
            cookies[i].setPath(?);
